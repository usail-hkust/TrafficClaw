"""
Agent-controlled simulation runner with checkpoint-based control for subway scheduling.
...
Currently supports:
- subway_scheduling: Subway scheduling control (headway and dwell_time optimization)
...
def run_checkpoint_based_simulation(
    ...
    """
    Run simulation with checkpoint-based control for LLM agent optimization of subway scheduling.

    Currently supports subway_scheduling only. Other control types (signal_timing,
    ...
    if use_wandb and WANDB_AVAILABLE:
        ...
        if control_modules is None:
            control_modules = ['subway_scheduling']  # Default
        ...
        wandb_run = wandb.init(
            project=wandb_project or "sumo_subway_scheduling_control",
    ...
    try:
        ...
        print("=" * 80)
        print(f"Starting checkpoint-based simulation for Subway Scheduling Control")
        ...
        if control_modules is None:
            control_modules = ['subway_scheduling']  # Default to subway scheduling control only
        ...
        while remaining_duration > 0:
            ...
            if results.get("checkpoint_reached", False):
                ...
                num_items = f"{len(control_configs.get('subway_scheduling', {})) if control_configs else 0} subway routes"

                initial_prompt = f"""You are optimizing subway scheduling for a city metro network.
...
Begin your analysis."""
                ...
                if optimization_result.get("success"):
                    ...
                    # Backward compatibility: if final_control_configs not available, use final_subway_config
                    if not final_control_configs and optimization_result.get("final_subway_config"):
                        final_control_configs = {"subway_scheduling": optimization_result["final_subway_config"]}
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run SUMO simulation with checkpoint-based LLM agent control for subway scheduling"
    )
    parser.add_argument(
        "--config",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        nargs='+',
        default=['subway_scheduling'],
        choices=['subway_scheduling', 'signal_timing', 'bus_scheduling'],
        help="Control modules to enable. Available: subway_scheduling, signal_timing (TODO), bus_scheduling (TODO). "
             "Can specify multiple modules, e.g., --control-modules subway_scheduling signal_timing. "
             "(default: subway_scheduling)"
    )
    parser.add_argument(
        "--wandb",
