"""
Agent-controlled simulation runner with checkpoint-based control for ramp metering.
...
def format_simulation_time(seconds: float) -> Dict[str, Any]:
    ...
    if 6 <= hours < 11:
        ...
        time_period = "Night"
        period_description = "Night period (00:00-06:00). Low traffic levels. Ramp metering may be less restrictive."
...
def run_checkpoint_based_simulation(
    ...
    """
    Run simulation with checkpoint-based control for LLM agent optimization of ramp metering.
    
    Currently supports ramp_metering only. Other control types (subway_scheduling, 
    ...
    if use_wandb and WANDB_AVAILABLE:
        ...
        if control_modules is None:
            control_modules = ['ramp_metering']  # Default
        ...
        wandb_run = wandb.init(
            project=wandb_project or "sumo_ramp_metering_control",
    ...
    try:
        ...
        print("=" * 80)
        print(f"Starting checkpoint-based simulation for Ramp Metering Control")
        ...
        if control_modules is None:
            control_modules = ['ramp_metering']  # Default to ramp metering control only
        ...
        # Get ramp metering config for context
        ramp_metering_config = env.enabled_controls.get('ramp_metering', {}).get('config', {}) if hasattr(env, 'enabled_controls') else {}
        ...
        while remaining_duration > 0:
            ...
            if results.get("checkpoint_reached", False):
                ...
                # Check if queue_length is low enough to skip optimization
                queue_length = 0.0
                if module_metrics and 'ramp_metering' in module_metrics:
                    queue_length = module_metrics['ramp_metering'].get('avg_queue_len', 0.0)
                
                if queue_length < 0.1 or remaining_duration > 3600 * 13:
                    print(f"\n{'='*80}")
                    print(f"[Checkpoint {checkpoint_count}] Skipping LLM Agent Optimization")
                    print(f"{'='*80}")
                    print(f"  Queue length ({queue_length:.4f}) is below threshold (0.1), skipping optimization.")
                    ...
                    agent_context = {
                        ...
                        # Ramp and Highway-specific data
                    ...
                    if module_metrics and 'ramp_metering' in module_metrics:
                        rm_metrics = module_metrics['ramp_metering']
                        print(f"    - Ramp avg queue length: {rm_metrics.get('avg_queue_len', 'N/A'):.2f}" if isinstance(rm_metrics.get('avg_queue_len'), (int, float)) else f"    - Ramp avg queue length: N/A")
                        print(f"    - Ramp avg waiting time: {rm_metrics.get('avg_waiting_time', 'N/A'):.2f}s" if isinstance(rm_metrics.get('avg_waiting_time'), (int, float)) else f"    - Ramp avg waiting time: N/A")
                        # Display throughput (now represents vehicle count, not veh/h)
                        throughput = rm_metrics.get('throughput')
                        if throughput is not None and isinstance(throughput, (int, float)):
                            print(f"    - Ramp throughput: {int(throughput)} vehicles")
                        else:
                            print(f"    - Ramp throughput: N/A")
                    ...
                    # Get ramp count
                    num_ramps = len(ramp_metering_config) if ramp_metering_config else 0
                    
                    initial_prompt = f"""You are optimizing ramp metering control for a city transportation network.
...
Begin your analysis."""
                    ...
                    if optimization_result.get("success"):
                        ...
                        # Backward compatibility: if final_control_configs not available, use final_ramp_metering_config
                        if not final_control_configs and optimization_result.get("final_ramp_metering_config"):
                            final_control_configs = {"ramp_metering": optimization_result["final_ramp_metering_config"]}
                        ...
                        if final_control_configs:
                            ...
                            for module_name, module_config in final_control_configs.items():
                                if module_name == "ramp_metering":
                                    # Update ramp metering config
                                    from control_modules import get_control_module
                                    ramp_module = get_control_module("ramp_metering")
                                    if ramp_module:
                                        ramp_metering_config = module_config  # Update local variable
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run SUMO simulation with checkpoint-based LLM agent control for ramp metering"
    )
    parser.add_argument(
        "--config",
        type=str,
        default="sumo_config_highway/Upper_Manhattan_highway/Upper_Manhattan.sumocfg",
    ...
    parser.add_argument(
        ...
        type=float,
        default=1800,
        help="Checkpoint interval in seconds (default: 1800 = 30 minutes)"
    )
    parser.add_argument(
        "--step-seconds",
        type=int,
    ...
    parser.add_argument(
        ...
        type=str,
        nargs='+',
        default=['ramp_metering'],
        choices=['ramp_metering', 'signal_timing', 'subway_scheduling', 'bus_scheduling'],
        help="Control modules to enable. Available: ramp_metering, signal_timing, subway_scheduling (TODO), bus_scheduling (TODO). "
             "Can specify multiple modules, e.g., --control-modules ramp_metering signal_timing. "
             "(default: ramp_metering)"
    )
    parser.add_argument(
        "--wandb",
