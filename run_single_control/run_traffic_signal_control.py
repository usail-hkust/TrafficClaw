"""
Agent-controlled simulation runner with checkpoint-based control.
...
Currently supports:
- signal_timing: Traffic signal timing control
...
def run_checkpoint_based_simulation(
    ...
    """
    Run simulation with checkpoint-based control for LLM agent optimization.
    
    Currently supports signal_timing only. Other control types (subway_scheduling, 
    ...
    # Maintain global travel time list (all vehicles from start to end of simulation)
    global_travel_times = []  # List of all vehicle travel times across entire simulation
    
    # Maintain global waiting time list (all checkpoints)
    global_waiting_times = []  # List of checkpoint waiting times for comparison with policy simulation
    ...
    if use_wandb and WANDB_AVAILABLE:
        ...
        wandb_run = wandb.init(
            project=wandb_project or "sumo_traffic_signal_control",
    ...
    try:
        ...
        print("=" * 80)
        print(f"Starting checkpoint-based simulation")
        ...
        agent = LLMAgent(
            ...
            available_control_modules=control_modules,  # Pass available control modules
        ...
        while remaining_duration > 0:
            ...
            if results.get("checkpoint_reached", False):
                ...
                # Update global travel time list with vehicles that arrived in this checkpoint
                ...
                if checkpoint_avg_travel_time is not None and checkpoint_arrived_count > 0:
                    # Get individual travel times for vehicles that arrived in this checkpoint
                    current_arrived_tt = env.get_arrived_vehicle_travel_times() if hasattr(env, 'get_arrived_vehicle_travel_times') else {}
                    # Add travel times to global list (we'll track them per checkpoint)
                    # Note: We can't easily get individual times per checkpoint, so we'll use the average
                    # For now, we'll maintain a list of checkpoint averages
                ...
                # Update global waiting time list with waiting time at this checkpoint
                ...
                if checkpoint_avg_travel_time is not None:
                    print(f"    - Checkpoint avg travel time ({checkpoint_arrived_count} vehicles): {checkpoint_avg_travel_time:.2f}s")
                
                # Display waiting time information
                print(f"\n  Waiting Time Metrics:")
                print(f"    - Global avg waiting time (all waiting vehicles): {results.get('avg_waiting_time', 0):.2f}s")
                if checkpoint_avg_waiting_time is not None:
                    print(f"    - Checkpoint avg waiting time ({checkpoint_waiting_count} vehicles): {checkpoint_avg_waiting_time:.2f}s")
                ...
                checkpoint_info = {
                    ...
                    "avg_travel_time": results.get("avg_travel_time", 0),  # Global average
                    "checkpoint_avg_travel_time": checkpoint_avg_travel_time,  # Checkpoint-specific average
                    "checkpoint_arrived_count": checkpoint_arrived_count,  # Vehicles arrived in this checkpoint
                    "avg_waiting_time": results.get("avg_waiting_time", 0),  # Global average
                    "checkpoint_avg_waiting_time": checkpoint_avg_waiting_time,  # Checkpoint-specific average
                    "checkpoint_waiting_count": checkpoint_waiting_count  # Vehicles waiting at checkpoint
                }

                # Add module metrics to checkpoint info
                if module_metrics:
                ...
                if wandb_run:
                    ...
                    # Add checkpoint-specific metrics if available
                    if checkpoint_avg_travel_time is not None:
                ...
                # Use checkpoint-specific travel time for comparison (not global average)
                # Use extract_configs_only so initial_best_result is JSON-serializable (no module instances)
                checkpoint_avg_tt = checkpoint_avg_travel_time if checkpoint_avg_travel_time is not None else results.get("avg_travel_time", 0)
                checkpoint_simulation_result = {
                    "success": True,
                    "stats": {
                        "total_departed": results.get("total_departed", 0),
                        "total_arrived": checkpoint_arrived_count,  # Use checkpoint-specific count
                        "avg_travel_time": checkpoint_avg_tt,  # Use checkpoint-specific average
                ...
                print(f"    - Checkpoint avg travel time ({checkpoint_arrived_count} vehicles): {checkpoint_avg_tt:.2f}s")
                print(f"    - Global avg travel time (all vehicles): {results.get('avg_travel_time', 0):.2f}s")
                print(f"    - Total arrived in checkpoint: {checkpoint_arrived_count}")
                ...
                initial_prompt = f"""You are optimizing traffic signal timing for a city transportation network.
...
- Checkpoint: {checkpoint_count}
- Number of intersections: {len(control_configs.get('signal_timing', {})) if control_configs else 0}
...
- **Your optimized signal timing will be applied from {time_info['time_string']} to {next_checkpoint_time_info['time_string']}**
...
Begin your analysis."""
                ...
                if optimization_result.get("success"):
                    ...
                    if final_control_configs:
                        ...
                        # Note: We don't save configs to file - each experiment starts fresh with get_default_config()
                        for module_name, module_config in final_control_configs.items():
                            print(f"    ✓ {module_name}: {len(module_config)} entries updated")
        ...
        if env:
            ...
            final_avg_waiting_time = env.get_average_waiting_time() if hasattr(env, 'get_average_waiting_time') else 0.0
            ...
            accumulated_results["global_travel_times"] = global_travel_times  # Store checkpoint travel time history
            accumulated_results["global_waiting_times"] = global_waiting_times  # Store checkpoint waiting time history
            ...
            # Calculate overall average waiting time across all checkpoints
            if global_waiting_times:
                # Calculate weighted average based on waiting vehicle count at each checkpoint
                total_waiting_time_sum = sum(wt["avg_waiting_time"] * wt["waiting_count"] for wt in global_waiting_times)
                total_waiting_count = sum(wt["waiting_count"] for wt in global_waiting_times)
                overall_avg_waiting_time = total_waiting_time_sum / total_waiting_count if total_waiting_count > 0 else 0.0
        ...
        if wandb_run:
            ...
            if accumulated_results.get("average_module_metrics"):
                ...
                if accumulated_results.get("final_avg_waiting_time"):
                    final_metrics["final/avg_waiting_time"] = accumulated_results["final_avg_waiting_time"]
                if accumulated_results.get("overall_avg_waiting_time"):
                    final_metrics["final/overall_avg_waiting_time"] = accumulated_results["overall_avg_waiting_time"]
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run SUMO simulation with checkpoint-based LLM agent control"
    )
    parser.add_argument(
        "--config",
        type=str,
        default="sumo_config/mzw/mzw.sumocfg",
    ...
    parser.add_argument(
        "--llm-model",
        type=str,
        default="siliconflow/MiniMaxAI/MiniMax-M2",
        help="LLM model name for agent (default: openai/o4-mini)"
    )
    parser.add_argument(
        "--max-agent-turns",
        type=int,
    ...
    parser.add_argument(
        ...
        type=str,
        nargs='+',
        default=['signal_timing'],
        choices=['signal_timing', 'subway_scheduling', 'bus_scheduling'],
        help="Control modules to enable. Available: signal_timing, subway_scheduling (TODO), bus_scheduling (TODO). "
             "Can specify multiple modules, e.g., --control-modules signal_timing subway_scheduling. "
             "(default: signal_timing)"
    )
    parser.add_argument(
        "--wandb",
    ...
    parser.add_argument(
        ...
        type=str,
        default='ChatCity',
        help="Wandb project name (default: sumo_traffic_signal_control)"
    )
    parser.add_argument(
