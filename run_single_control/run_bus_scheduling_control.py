"""
Agent-controlled simulation runner with checkpoint-based control for bus scheduling.
...
Currently supports:
- bus_scheduling: Bus scheduling control (headway and dwell_time optimization)
...
from utils.simulation_utils import (
    ...
    build_road_network_graphs
)
from utils.id_utils import generate_simulation_identifiers
from utils.traffic_state_collector import (
    init_traffic_states_file,
...
def run_checkpoint_based_simulation(
    ...
    """
    Run simulation with checkpoint-based control for LLM agent optimization of bus scheduling.

    Currently supports bus_scheduling only. Other control types (signal_timing,
    subway_scheduling) are planned for future implementation.
    ...
    if use_wandb and WANDB_AVAILABLE:
        ...
        if control_modules is None:
            control_modules = ['bus_scheduling']  # Default
        ...
        wandb_run = wandb.init(
            project=wandb_project or "sumo_bus_scheduling_control",
    ...
    try:
        ...
        print("=" * 80)
        print(f"Starting checkpoint-based simulation for Bus Scheduling Control")
        ...
        if control_modules is None:
            control_modules = ['bus_scheduling']  # Default to bus scheduling control only
        ...
        # Signal timing config will be initialized from control modules below
        # No need to initialize here separately - it will be reinitialized for all modules
        ...
        while remaining_duration > 0:
            ...
            if results.get("checkpoint_reached", False):
                ...
                agent_context = {
                    ...
                    "lane_dict": graphs.get("lane_dict"),  # Only intersection-related lanes (2586)

                    # Layer 1: Foundation Layer - Complete network graphs
                    # These contain ALL lanes/roads in the network, not just intersection-related
                    ...
                    # Convenience aliases for Foundation Layer data
                    ...
                    "transit_graph": env.transit_graph if hasattr(env, 'transit_graph') else None,
                    "bus_route_info": env.bus_route_info if hasattr(env, 'bus_route_info') else {},

                    # Simulation context
                    "traffic_states_filepath": str(traffic_states_filepath),
                    "simulation_id": simulation_id,  # Pass simulation_id to automatically filter data from current simulation
                    "current_configs": control_configs,  # All current module configurations as a dictionary
                    "checkpoint_path": checkpoint_path,
                    "config_path": config_path,  # SUMO config file path (for policy simulation)
                    "checkpoint_interval": checkpoint_interval,  # Checkpoint interval (for loading t-1 state)
                    "test_duration": checkpoint_interval,  # Test simulation duration
                    "remaining_duration": remaining_duration,  # Remaining simulation duration for run_counts
                    "use_gui": use_gui,  # Pass use_gui setting to match main simulation
                    "config_name": config_dir_name,  # Pass config_name for file naming
                    "llm_name": llm_model,  # Pass llm_name for file naming
                    "control_modules": control_modules,  # Pass control_modules for file naming
                ...
                # Task description for bus scheduling optimization
                task_title = "optimizing bus scheduling for a city bus network"
                task_description = """For task description, optimization objectives, configuration format, constraints, and examples, follow the bus_scheduling module's domain knowledge (provided via GET_CONTROL_API).

Available control modules: {modules}"""
                num_items = f"{len(control_configs.get('bus_scheduling', {})) if control_configs else 0} bus routes"

                initial_prompt = f"""You are {task_title}.
...
- Control items: {num_items}
- Traffic states available: {traffic_states_filepath.name}

{task_description.format(time_period=time_info['time_period'], modules=', '.join(control_modules))}
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run SUMO simulation with checkpoint-based LLM agent control for bus scheduling"
    )
    parser.add_argument(
        "--config",
        type=str,
    ...
    parser.add_argument(
        ...
        help="LLM model name for agent (default: openai/gpt-4o-mini)"
    )
    parser.add_argument(
        "--max-agent-turns",
        type=int,
    ...
    parser.add_argument(
        ...
        type=str,
        nargs='+',
        default=['bus_scheduling'],
        choices=['bus_scheduling', 'signal_timing', 'subway_scheduling'],
        help="Control modules to enable. Available: bus_scheduling, signal_timing (TODO), subway_scheduling (TODO). "
             "Can specify multiple modules, e.g., --control-modules bus_scheduling signal_timing. "
             "(default: bus_scheduling)"
    )
    parser.add_argument(
        "--wandb",
