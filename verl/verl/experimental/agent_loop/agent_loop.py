...
class AgentLoopOutput(BaseModel):
    ...
    extra_info: dict = {}
    """Extra info for reward computation (e.g., cumulative_reward for DeepCity)"""
...
class AgentLoopWorker:
    ...
    async def generate_sequences(self, batch: DataProto) -> DataProto:
        ...
        #   Read sample_id field (if exists)
        sample_ids = batch.non_tensor_batch.get("sample_id", None)
        
        #   Read global batch_index (calculated by ray_trainer.py before repeat)
        # This ensures each sample has the correct intra-batch position regardless of num_workers
        global_batch_indices = batch.non_tensor_batch.get("global_batch_index", None)
        
        #   Read is_padded flag (added by ray_trainer.py during padding)
        # Padded samples should not trigger SUMO interaction to avoid interfering with Master's checkpoint_in_use mechanism
        is_padded_flags = batch.non_tensor_batch.get("is_padded", None)
        ...
        for i, (agent_name, messages, trajectory) in enumerate(zip(agent_names, raw_prompts, trajectory_info, strict=True)):
            # Get current sample's sample_id
            sample_id = sample_ids[i] if sample_ids is not None else None
            
            #   Use global batch_index (if exists), otherwise fallback to enumerate index
            if global_batch_indices is not None:
                batch_index = int(global_batch_indices[i])
            else:
                batch_index = i
            
            #   Check if it's a padded sample
            is_padded = bool(is_padded_flags[i]) if is_padded_flags is not None else False
            
            tasks.append(
                asyncio.create_task(self._run_agent_loop(agent_name, messages.tolist(), sampling_params, trajectory, sample_id, batch_index, is_validate, is_padded))
            )
    ...
    async def _run_agent_loop(
        ...
        is_validate: bool = False,  #   validation mode flag
        is_padded: bool = False,  #   padded sample flag (skip SUMO interaction)
    ) -> AgentLoopOutput:
        with rollout_trace_attr(
            ...
            #   Pass sample_id, batch_index, is_validate, is_padded to agent_loop.run()
            output = await agent_loop.run(messages, sampling_params, sample_id=sample_id, batch_index=batch_index, is_validate=is_validate, is_padded=is_padded)
            return output
    ...
    def _postprocess(self, inputs: list[AgentLoopOutput]) -> DataProto:
        ...
        #   Collect extra_info (used to pass cumulative_reward and other info to RewardManager)
        extra_infos = [input.extra_info for input in inputs]
        
        return DataProto(
            batch=batch, 
            non_tensor_batch={
                "__num_turns__": num_turns,
                "extra_info": np.array(extra_infos, dtype=object),  #   Add extra_info
            }, 
