    ...
    if prediction_or_decision == 'Prediction':
        ...
        if task == 'traffic_signal_control':
            ...
            if data['stage'] == 'stage1':
                new_question = new_question.replace(
                        '<env_changes>',
                        f"Newly connected regions: {round(data['environment_feedback'][option2road[data['decision'][0]]]['newly_connected_regions'], 2)}\n"
                        f"Maximum number of regions can be connected next: {round(data['environment_feedback'][option2road[data['decision'][0]]]['potential_highest_connected_regions_via_new_roads'], 2)}"
                )
            elif data['stage'] == 'stage2':
                new_question = new_question.replace(
                        '<env_changes>',
                        f"Distance deduction among regions: {round(data['environment_feedback'][option2road[data['decision'][0]]]['distance_reduction_among_regions'], 2)}\n"
                        f"Maximum distance deduction via building new roads: {round(data['environment_feedback'][option2road[data['decision'][0]]]['potential_highest_distance_reduction_via_new_roads'], 2)}"
                )
